...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <AppShell name={name} role={role} locationName={location?.name ?? (role === "staff" ? "My published schedule" : "No assigned location")} timezone={location?.timezone ?? actor.staffProfile?.primaryTimezone ?? "Timezone unavailable"} notifications={notifications} notificationMode={preferences.notificationMode}>{children}</AppShell>;
}
