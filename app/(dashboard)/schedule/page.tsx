...
export default async function SchedulePage({ searchParams }: { searchParams: Promise<{ week?: string; location?: string }> }) {
  ...
  return <section className="px-4 py-6 sm:px-6 lg:px-8"><ScheduleHeading weekStart={weekStart} eyebrow="Manager board" title={selected?.name ?? "Schedule workspace"} />
    <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_18rem]"><div className="overflow-x-auto border bg-white"><div className="grid min-w-[760px] grid-cols-7">
      {days.map((day) => <div key={day.iso} className="min-h-[28rem] border-r last:border-r-0"><div className="border-b bg-[var(--surface-subtle)] px-3 py-3"><p className="text-xs font-semibold uppercase tracking-[0.08em]">{day.day}</p><p className="font-mono text-[10px] text-muted-foreground">{day.date}</p></div><div className="space-y-2 p-2">{shifts.filter((shift) => new Intl.DateTimeFormat("en-CA", { timeZone: selected?.timezone, year: "numeric", month: "2-digit", day: "2-digit" }).format(shift.startsAt) === day.iso).map((shift) => <div key={shift.shiftId} className="border-l-2 border-primary bg-[var(--surface-subtle)] p-2.5"><p className="text-xs font-semibold">{shift.skillName}</p><p className="mt-1 font-mono text-[10px] text-muted-foreground">{shift.startsAt.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", timeZone: selected?.timezone })} · {shift.headcount} needed</p></div>)}</div></div>)}
