...
function ScheduleHeading({ weekStart, eyebrow, title, action }: { weekStart: string; eyebrow: string; title: string; action?: React.ReactNode }) {
  return <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-primary">{eyebrow}</p><h1 className="mt-1 font-heading text-4xl uppercase leading-none">{title}</h1></div><div className="flex flex-wrap items-end gap-5"><div className="border-l-2 border-primary pl-3"><p className="text-xs text-muted-foreground">Week of</p><p className="font-mono text-sm font-medium">{weekStart}</p></div>{action}</div></header>;
}
