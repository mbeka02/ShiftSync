...
import { AlertTriangle, CalendarCheck2, CalendarRange, CheckCircle2, Clock3, MapPin } from "lucide-react";
...
function ScheduleHeading({ weekStart, eyebrow, title, action }: { weekStart: string; eyebrow: string; title: string; action?: React.ReactNode }) {
  ...
  return <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-primary">{eyebrow}</p><h1 className="mt-1 font-heading text-4xl uppercase leading-none">{title}</h1></div><div className="flex flex-wrap items-center gap-3 lg:pb-0.5"><div className="flex h-9 items-center gap-2.5 border-r border-[var(--border-strong)] pr-4"><CalendarRange className="size-4 text-primary" /><div><p className="font-mono text-[8px] uppercase tracking-[0.12em] text-muted-foreground">Week of</p><p className="text-sm font-semibold leading-tight">{weekLabel}</p></div></div>{action}</div></header>;
}
